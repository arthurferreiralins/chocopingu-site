module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    try {
      const resp = await fetch(`${SB_URL}/rest/v1/avaliacoes?order=created_at.desc&limit=200`, {
  ...
  if (req.method === 'PATCH') {
    ...
    try {
      const resp = await fetch(`${SB_URL}/rest/v1/avaliacoes?id=eq.${encodeURIComponent(id)}`, {
        method: 'PATCH',
        headers: {
          apikey: SB_KEY,
